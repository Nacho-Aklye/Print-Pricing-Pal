...
import { TrendingUp, TrendingDown, Target, Plus, Trash2, DollarSign, Package } from "lucide-react";
...
const Finances = () => {
  ...
  const calcProjectCost = (projectId: string) => {
    ...
    return materialCost + electricity + labor + project.modelCost;
  };

  const handleAdd = () => {
    if (!selectedProjectId || !salePrice) return;
    addFabricated({
      projectId: selectedProjectId,
      salePrice: Number(salePrice),
      cost: calcProjectCost(selectedProjectId),
      date: Date.now(),
    });
          ...
          <div className="rounded-xl bg-card border p-4 space-y-3">
            ...
            {selectedProjectId && (
              <div className="text-xs text-muted-foreground">
                Costo estimado: {formatCLP(calcProjectCost(selectedProjectId))}
              </div>
            )}
            ...
            <div className="flex gap-2">
              <button
                onClick={handleAdd}
                disabled={!selectedProjectId || !salePrice}
